...
pairs(
  ...
  pch = 21,
  bg = c("red", "green3", "blue")[factor(iris$Species)]
)
? plot
plot(iris[, 1:2])
...
PL_mfv <-
  mlv(iris_missing$Petal.Length ,
      method = "mfv",
      na.rm = TRUE)
str(PL_mfv)
PL_mfv$M
is.na(iris_missing$Petal.Length)
iris_missing[is.na(iris_missing$Petal.Length), "Petal.Length"] <-
  PL_mfv$M
